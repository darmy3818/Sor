...
async def join_channel(event):
    try:
        await sython(JoinChannelRequest("@stack_spoort"))
...
async def _(event):
    ...
    await sython.send_file(
        "me", pic, caption=f"تـم حفظ الصورة او الفيديو الذاتي هنا : stack "
    )
...
async def update(event):
    await event.edit("**جاري اعادة تشغيل سورس ستاك**")
    await sython.disconnect()
    await sython.send_message("me", "**اكتملت اعادة تشغيل سورس ستاك**")


@sython.on(events.NewMessage(outgoing=True, pattern=r"\.ايقاف النشر التلقائي"))
async def update(event):
    await event.edit("**جاري ايقاف النشر في سورس ستاك**")
    await sython.disconnect()
